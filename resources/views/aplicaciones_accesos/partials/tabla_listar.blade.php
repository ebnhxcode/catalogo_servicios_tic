<table class="table table-striped table-hover table-sm">
   ...
   <tbody>
   <tr v-for="aa in filterBy(aplicaciones_accesos, filtro_head)"
       @dblclick.prevent="editar(aa.id_aplicacion_acceso)">

      <template v-if="id_en_edicion != aa.id_aplicacion_acceso || modal_actualizar_activo == true">
      ...
      <template v-else>
         <td>
            <span v-clipboard="aa.id_aplicacion_acceso" class="btn btn-light"
                  data-placement="top" data-toggle="tooltip" title="Clic para copiar el id">
               Id servidor acceso: @{{ aa.id_aplicacion_acceso }}
            </span>
         </td>
      ...
      <td>
         <button class="btn btn-sm btn-primary"
                 v-show="id_en_edicion != aa.id_aplicacion_acceso && id_en_edicion == null && modal_actualizar_activo == false"
                 data-placement="top" data-toggle="tooltip" title="Editar desde aquí"
                 @click.prevent="editar(aa.id_aplicacion_acceso)">
            <i class="fa fa-edit"></i>
         </button>
         <button class="btn btn-sm btn-success" v-show="id_en_edicion == aa.id_aplicacion_acceso && modal_actualizar_activo == false"
         ...
         <button class="btn btn-sm btn-warning"
                 ...
                 @click.prevent="mostrar_modal_actualizar(aa.id_aplicacion_acceso)"
                 v-show="id_en_edicion == null">
            <i class="fa fa-cogs" ></i>
         </button>
         {{--
         <button class="btn btn-sm btn-danger" v-show="id_en_edicion == null"
                 @click.prevent="eliminar(aa.id_aplicacion_acceso)"
         ...
         <button class="btn btn-sm btn-secondary"
                 ...
                 v-show="id_en_edicion == aa.id_aplicacion_acceso">
            <i class="fa fa-close"></i>
         </button>
   ...
   <tr v-if="aplicaciones_accesos && aplicaciones_accesos.length == 0 || filterBy(aplicaciones_accesos, filtro_head).length == 0">
