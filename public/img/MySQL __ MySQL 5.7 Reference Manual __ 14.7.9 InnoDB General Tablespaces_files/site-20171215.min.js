...
_.hooks.run('before-highlight',env);if(async&&_self.Worker){var worker=new Worker(_.filename);worker.onmessage=function(evt){env.highlightedCode=evt.data;_.hooks.run('before-insert',env);env.element.innerHTML=env.highlightedCode;callback&&callback.call(env.element);_.hooks.run('after-highlight',env);_.hooks.run('complete',env)};worker.postMessage(JSON.stringify({language:env.language,code:env.code,immediateClose:!0}))}
...
(function($)
{$.doc=function(opt)
{this.defaults={'navStickyMinWidth':860,'mobile':!1,'docId':0,'highlight':!1}
this.navObj=$('#sub-header');this.headerObj=$('#l1-nav-container');this.bodyObj=$('body');this.footerObj=$('footer');this.docTocObj=$('#docs-sidebar-toc');this.docsMainObj=$('#docs-main');this.docsMainInner=$('#docs-main-inner');this.docTocInner=$('#docs-toc-inner');this.window=$(window);this.document=$(document);this.tocPosition=null;this.copyTimer=null;this.tables=[];this.theadMagicNumber=4;this.theadTimer=null;this.theadMinRows=10;this.options=$.extend({},this.defaults,opt);this.init=function()
{var self=this;$('.docs-sidebar-nav a.expandable').doctoc(self);$('#docs-hide-nav').click(function(e){self.docTocObj.hide();self.docsMainObj.removeClass('has-toc').addClass('no-toc');$('#docs-show-nav').show();self.docsMainObj.css('min-height','100px');$.get('/doc/sidebar/?visible=0&d='+self.options.docId,{});return!1});$('.docs-sidebar-section').click(function(e){var div=$(this).next();if(div.is(':hidden')){div.slideDown('fast',function(){self.adjustHeight()})}else{div.slideUp('fast',function(){self.adjustHeight()})}
...
{var previousCall=new Date().getTime();return function(){var time=new Date().getTime();if((time-previousCall)>=delay){previousCall=time;callback.apply(null,arguments)}}}
...
{var self=this;$(prefix+' #docs-comment-show-form-btn').click(function(e){$(this).hide()
$('#add-comment').show();$('html, body').animate({scrollTop:$('#add-comment').offset().top},'slow');return!1});$(prefix+' #docs-comment-hide-form-btn').click(function(e){$('#add-comment').hide();$('#docs-comment-show-form-btn').show()
self.adjustHeight();return!1});$(prefix+' .docs-comment-cancel-btn').click(function(e){var id=$(this).closest('form').find('.id-input').val();$('#c'+id+' .docs-comment-item-edit').html();$('#c'+id+' .docs-comment-item-edit').hide();$('#c'+id+' .docs-comment-item-text').show();self.adjustHeight();return!1});$(prefix+' .json-input').val(1);$(prefix+' .docs-comment-delete-link').click(function(e){if(confirm('Are you sure you want to delete this comment?')){var id=$(this).attr('id').substring(1);self.showCommentLinkSpinner(id);$.ajax({type:'GET',url:'/doc/delete-comment/',data:{'id':id,'json':1},dataType:'json',success:function(data,msg){if(!data.success){alert(data.results)}else{$('#c'+id).remove();self.adjustHeight()}
...
return!1});$(prefix+' .docs-comment-edit-link').click(function(e){var id=$(this).attr('id').substring(1);self.showCommentLinkSpinner(id);$.ajax({type:'GET',url:'/doc/comment/',data:{'id':id,'json':1},dataType:'json',success:function(data,msg){if(!data.success){alert(data.results)}else{$('#c'+id+' .docs-comment-item-edit').html(data.results).show();var path=$('#add-comment .path-input').val();$('#c'+id+' .path-input').val(path);self.initComments('#c'+id+' .docs-comment-item-edit');$('#c'+id+' .docs-comment-item-text').hide();self.adjustHeight()}
self.hideCommentLinkSpinner(id)},error:function(xhr,status_text,error_thrown){alert('An unexpected error was encountered.');self.hideCommentLinkSpinner(id)}});return!1});$(prefix+' .docs-comment-submit-btn').click(function(e){e.preventDefault();var form=$(this).closest('form');var data=form.serializeArray();var id=$(form).find('.id-input').val();$(form).find('.docs-comment-btn').prop('disabled',!0).addClass('disabled');$.ajax({type:'POST',url:'/doc/comment/',data:form.serializeArray(),dataType:'json',success:function(data,msg){if(!data.success){alert(data.results)}else{if(id==''){$('#comment-listing').append(data.results);var cid=$('#comment-listing').children().last().attr('id');self.initComments('#'+cid);$('#docs-comment-hide-form-btn').trigger('click');$(form).find('textarea').val('')}else{$('#c'+id).replaceWith(data.results);self.initComments('#c'+id)}
self.adjustHeight()}
$(form).find('.docs-comment-btn').prop('disabled',!1).removeClass('disabled')},error:function(xhr,status_text,error_thrown){alert('An unexpected error was encountered.')}});return!1})}
this.showCommentLinkSpinner=function(id)
{var spinner='<span class="icon-spinner pulse"></span>';$('#c'+id+' .docs-comment-item-header-spinner').html(spinner).show();$('#c'+id+' .docs-comment-item-header-links').hide()}
this.hideCommentLinkSpinner=function(id)
{$('#c'+id+' .docs-comment-item-header-spinner').html('').hide();$('#c'+id+' .docs-comment-item-header-links').show()}
this.parseInt=function(value)
{var result=parseInt(value,10);return isNaN(result)?0:result}
this.clearHeights=function()
{this.docTocObj.css('max-height','none');this.docsMainObj.css('min-height','none')}
this.adjustHeight=function()
{var width=$(window).width();if(width<=this.options.navStickyMinWidth||this.docTocObj.is(':hidden')){this.clearHeights();return this}
...
{if($(window).scrollTop()>=this.headerObj.height()+this.navObj.height()){var width=$(window).width();if((!this.tocPosition||this.tocPosition=='static')&&width>this.options.navStickyMinWidth){this.bodyObj.addClass('has-sticky-toc');this.docTocObj.addClass('sticky');this.tocPosition='fixed'}else if(this.tocPosition=='fixed'&&width<=this.options.navStickyMinWidth){this.bodyObj.removeClass('has-sticky-toc');this.docTocObj.removeClass('sticky');this.docTocObj.scrollTop(0);this.tocPosition='static'}
...
{this.on('click',function(e){var chevron=this;if($(this).hasClass('loaded')){var icon=$(this).find('span');var submenu=$(this).parent().parent().parent().find('.docs-submenu')[0];if(icon.hasClass('icon-chevron-down')){icon.removeClass('icon-chevron-down').addClass('icon-chevron-right');$(submenu).slideUp('fast',function(){doc.adjustHeight()})}else{icon.removeClass('icon-chevron-right').addClass('icon-chevron-down');$(submenu).slideDown('fast',function(){doc.adjustHeight()})}}else{var icon=$(this).find('span');var link=$(this).parent().parent().find('.docs-sidebar-nav-link a')[0];var submenu=$(this).parent().parent().parent().find('.docs-submenu')[0];var href=$(link).attr('href');var parts=$('.doctoc').attr('id').split('-');var docVersionId=parts[1];$(submenu).after('<div class="docs-toc-loading"><span class="icon-spinner pulse"></span>&nbsp; Loading</div>');$.ajax({type:'GET',url:'/doc/toc/',data:{'id':docVersionId,'page':href},dataType:'json',success:function(data,msg){if(!data.success){$(submenu).find('.docs-toc-loading').html('Error Loading Nav Items')}else{$(submenu).next().remove();$(submenu).html(data.results);$(submenu).find('a.expandable').doctoc(doc);$(chevron).addClass('loaded');icon.removeClass('icon-chevron-right').addClass('icon-chevron-down');$(submenu).slideDown('fast',function(){doc.adjustHeight()})}},error:function(xhr,status_text,error_thrown){}})}
e.stopPropagation();return!1});return this}})(jQuery);
(function($){$.fn.loadVersion=function(ga)
{$('#'+ga).mask('Loading ...');var os=$('#os-'+ga).val();var osva=$('#osva-'+ga).val();var url=$(this).attr('href');if(typeof url==='undefined'||url===!1){url=$('#dl-'+ga).attr('action');var version=$('#version-'+ga).val();var params={tpl:'version',os:os,version:version,osva:osva}}else{var params={tpl:'version',os:os,osva:osva}}
$.get(url,params).done(function(data){$('#'+ga).html(data);$('#'+ga).unmask()});return!1};$.fn.loadPlatform=function(ga)
{var url=$('#dl-'+ga).attr('action');var os=$('#os-'+ga).val();var osva=$('#osva-'+ga).val();$('#os-version-'+ga).mask('Loading ...');var version=$('#version-'+ga).val();$.get(url,{tpl:'platform',os:os,version:version,osva:osva}).done(function(data){$('#os-version-'+ga).html(data);$('#os-version-'+ga).unmask()});return!1};$.fn.loadFiles=function(ga)
{var url=$('#dl-'+ga).attr('action');var os=$('#os-'+ga).val();var osva=$('#osva-'+ga).val();if(os==0){return!1}
$('#files-'+ga).mask('Loading ...');var version=$('#version-'+ga).val();$.get(url,{tpl:'files',os:os,version:version,osva:osva}).done(function(data){$('#files-'+ga).html(data);$('#files-'+ga).unmask()});return!1};$.fn.loadArchiveVersion=function()
{$('#downloads').mask('Loading ...');var url=$('#dl').attr('action');var os=$('#os').val();var osva=$('#osva').val();var version=$('#version').val();var params={tpl:'version',os:os,version:version,osva:osva};$.get(url,params).done(function(data){$('#downloads').html(data);$('#downloads').unmask()});return!1};$.fn.loadArchivePlatform=function()
{var url=$('#dl').attr('action');var os=$('#os').val();var osva=$('#osva').val();var version=$('#version').val();if(os==0){return!1}
$('#os-version').mask('Loading ...');$.get(url,{tpl:'platform',os:os,version:version,osva:osva}).done(function(data){$('#os-version').html(data);$('#os-version').unmask()});return!1};$.fn.loadArchiveFiles=function()
{var url=$('#dl').attr('action');var os=$('#os').val();var osva=$('#osva').val();var version=$('#version').val();if(os==0){return!1}
...
{this.defaults={'keywords':'','priority':'','severity':'','page':1,'sort':'last_modified','sort_dir':'DESC','controller_url':'/worklog'}
this.loading=!1;this.options=$.extend({},this.defaults,opt);this.viewed=[];this.init=function()
{var self=this;$('#task_search').submit(function(event){event.preventDefault();if($('#k').val()){self.options.sort='';self.options.sort_dir=''}else{self.options.sort=$('#sc').val();self.options.sort_dir=$('#sd').val()}
...
{var self=this;$('#pagination a, .sort_link').click(function(){var href=$(this).attr('href');$('#sc').val(self.getQueryParam(href,'sc'));$('#sd').val(self.getQueryParam(href,'sd'));var parts=href.split('?');self.refresh('Loading',parts[1]);return!1});this.initTaskLinks('.task_link')}
this.initTaskLinks=function(selector)
{var self=this;$(selector).click(function(){self.loadTask($(this).attr('href'));return!1})}
this.scrollTabsTop=function()
{var self=this;$('.ui-tabs-nav a').click(function(){var id=$(this).attr('href');if(!self.viewed[id]){$(id).scrollTop(0);self.viewed[id]=1}
return!1})}
this.loadTask=function(href)
{var self=this;jQuery.facebox(function(){jQuery.get(href+'&facebox=1',function(data){jQuery.facebox(data);self.viewed=[];var current=$('.ui-tabs-nav li.ui-state-active a').attr('href');$(current).scrollTop(0);self.viewed[current]=1;self.scrollTabsTop();self.initTaskLinks('.tab_task_link')})})}
this.getQueryParam=function(query,name)
{var regex=new RegExp("[\\?&]"+name+"=([^&#]*)");var matches=regex.exec(query);return(matches==null)?'':matches[1]}
this.refresh=function(message,params)
{if(this.loading){return}
this.loading=!0;var self=this;$('#results_container').mask(message+' ...');$.ajax({type:'GET',url:this.options.controller_url+'/search/',data:(params===!1)?this.getParams(!0):params,dataType:'json',success:function(data,msg){$('#results_container').unmask();$('#results_container').html(data.results);self.initLinks();self.loading=!1},error:function(xhr,status_text,error_thrown){$('#results_container').unmask();self.loading=!1}})};this.getParams=function(json)
{var self=this;var params={'k':self.options.keywords,'p':self.options.priority,'s':self.options.status,'sc':self.options.sort,'sd':self.options.sort_dir,'pg':self.options.page}
return(json?params:$.param(params))}
this.getIntValue=function(id)
{var i=parseInt($('#'+id).val().replace(/\D/g,''),10);return isNaN(i)?0:i}}})(jQuery);
(function($)
{$.content=function(opt)
{this.defaults=$.extend({'main':0,'type':0,'pathPrefix':''},opt);this.init=function()
{var self=this;$(window).hashchange(function(){self.load()})
if(location.href.split("#")[1]){$('#content-container').html('<br /><br /><br /><br /><br />');$(window).hashchange()}else{this.initEvents()}};this.getHashData=function()
{var hash=(location.href.split("#")[1]||"");hash=hash.replace(/^#?(.*)$/,'$1');hash=jQuery.trim(hash);if(hash!=''){var parts=hash.split('-');if(parts.length==3){return{'lang':parts[0],'type':parts[1],'topic':parts[2],'main':this.defaults.main}}}
return{'lang':this.defaults.lang,'type':this.defaults.type,'topic':0,'main':this.defaults.main}}
this.load=function()
{var self=this;$('#content-container').mask('Loading ...');var prefix=this.defaults.pathPrefix;$.ajax({type:'GET',url:prefix+'/content/search/',data:this.getHashData(),dataType:'json',success:function(data,msg){$('#content-container').unmask();if(!data.success){$("#err_dialog").html(message);$("#err_dialog").dialog({modal:!0})}else{$('#content-container').html(data.content);self.initEvents()}},error:function(xhr,status_text,error_thrown){$('#content-container').unmask()}})};this.initEvents=function()
{var self=this;$('#type, #topic, #lang').change(function(){var lang=self.getStrValue('lang');var type=self.getIntValue('type');var topic=self.getIntValue('topic');window.location.hash=lang+'-'+type+'-'+topic});$('.content-more').click(function(){var id=$(this).attr('id');$('.'+id).show();$(this).hide();return!1})}
this.getIntValue=function(id)
{var i=parseInt($('#'+id).val().replace(/\D/g,''),10);return isNaN(i)?0:i};this.getStrValue=function(id)
{var str=$('#'+id).val().replace(/[^a-z]/g,'');return str}}})(jQuery);
(function($)
{$.rotator=function()
{this.numBanners=null;this.interval=14000;this.timeoutId=null;this.running=!1;this.current=null;this.loaded=[];this.init=function()
{var self=this;$('.fp-banner-dots a').click(function(){self.running=!1;clearTimeout(self.timeoutId);var id=$(this).attr('id');var num=parseInt(id.substring(id.length-1));self.rotateBanner(num);return!1});var currentId=$('#banners .current').attr('id');this.numBanners=$('.fp-banner').length;this.current=parseInt(currentId.substring(currentId.length-1));this.loaded[this.current]=!0;if(this.numBanners==1){$('.fp-banner-dots').hide()}else{this.running=!0;this.preLoadNext();this.timeoutId=setTimeout(function(){self.rotateBanner(self.getNextBannerNumber())},this.interval)}};this.rotateBanner=function(num)
...
(function($)
{$.slider=function()
{this.numLogos=null;this.logoWidth=120;this.first=null;this.init=function()
{var self=this;this.first=1;this.numLogos=$('#fp-logos a').length;$('.fp-logo-container .next').click(function(){var remaining=self.numLogos-self.first-self.getVisibleLogos()+1;if(remaining>0){$('#fp-logos').animate({'left':'-'+(self.first*self.logoWidth)+'px'},'fast');self.first++}
...
$('#server_count').bind('keypress',function(e){if(e.keyCode==13){$(this).blur();return!1}})};this.refresh=function(top)
{var self=this;var msg=this.options.lead_captured?self.options.msg_calculating:self.options.msg_validating;if(top){var new_position=$('#tco_graph').offset();window.scrollTo(new_position.left,new_position.top);var mask_id='#tco_graph'}else{var mask_id='#tco_form'}
$(mask_id).mask(msg+' ...');$.ajax({type:'GET',url:this.options.controller_url+'/calculate/',data:this.getParams(!0),dataType:'json',success:function(data,msg){$(mask_id).unmask();if(!data.success){self.valid=!1;self.clearHighlight();self.displayError(data.failed_constraints)}else{self.valid=!0;self.last_error='';self.clearHighlight();if(self.options.lead_captured){$('#tcotable').html(data.table);$('#graph_img').attr('src',data.graph)}}},error:function(xhr,status_text,error_thrown){$(mask_id).unmask()}})};this.getParams=function(json)
{var self=this;var editions=new Array();$.each(this.options.isv_ids,function(index,value){editions.push(self.getIntValue('edition_'+value))});var params={'server_count':this.options.server_count,'cpu_count':this.options.cpu_count,'core_count':this.options.core_count,'years':this.options.years,'editions':editions,'hash':$('#tco_hash_value').val()}
return(json?params:decodeURIComponent($.param(params)))}
this.getIntValue=function(id)
{var i=parseInt($('#'+id).val().replace(/\D/g,''),10);return isNaN(i)?0:i};this.displayError=function(failed_constraints)
...
{$('#edition_'+this.options.isv_ids.join(', #edition_')).css('color','black');$('#edition_'+this.options.isv_ids.join(' option, #edition_')+' option').css('color','black')};this.submitForm=function()
{if(this.valid){$('#tco_calculate_btn').attr('disabled','disabled');$('#tco').submit()}else{$("#err_dialog").html(this.last_error);$("#err_dialog").dialog({modal:!0})}};this.print=function()
{window.open(this.options.controller_url+'/print/?'+this.getParams(!1),'_blank')}}})(jQuery);
...
lastScrollTop=st}};stickyNav();$(window).scroll(function(){stickyNav()})}
...
if($(this).width()<=navStickyMinWidth&&!created){setupSmallScreen()}});if(!$.browser.mobile&&!docTocObj.length&&!$('#fp-docs-banner').length){var scrollTo=function(hash){var newTop=null;var width=$(window).width();var offset=(width>navStickyMinWidth)?50:10;if($(hash).length){newTop=$(hash).offset().top-offset}else if($('a[name='+hash.substr(1)+']').length){newTop=$('a[name='+hash.substr(1)+']').offset().top-offset}
