...
var CargoController = new Vue({
   ...
   watch: {
      ...
      cargos: function cargos(_cargos) {
         var self = this;
         this.excel_json_datos = [];
         return _cargos.map(function (cargo, index) {
   ...
   methods: {
      ...
      inicializar: function inicializar() {
         ...
         this.$http.get('/cargos').then(function (response) {
            // success callback
      ...
      editar: function editar(id_cargo) {
      ...
      guardar_editado: function guardar_editado() {
         ...
         this.$http.put('/' + this.nombre_ruta + '/' + this.cargo.id_cargo, this.cargo).then(function (response) {
            // success callback

            if (response.status == 200) {
               if (!_this2.es_null(response.body.cargo)) {
      ...
      eliminar: function eliminar(id_cargo) {
         var _swal,
             _this3 = this;
         ...
         __WEBPACK_IMPORTED_MODULE_0_sweetalert2___default()((_swal = {
            ...
            if (result.value) {
               ...
               _this3.$http.delete('/' + _this3.nombre_ruta + '/' + id_cargo).then(function (response) {
                  if (response.status == 200) {
      ...
      ordenar_lista: function ordenar_lista(columna) {
         this.cargos = _.orderBy(this.cargos, columna, this.orden_lista);
      }
