...
var ServicioController = new Vue({
   ...
   data: function data() {
      return {
         ...
         'tabla_campos': {
            ...
            'nom_servicio': { 'visibility': true, 'value': null },
            'det_servicio': { 'visibility': false, 'value': null },
            'n_aplicaciones': { 'visibility': false, 'value': null },
            'n_servidores': { 'visibility': false, 'value': null },
            //'id_actividad':{'visibility':false,'value':null},
            'nom_actividad': { 'visibility': false, 'value': null },
            //'id_usuario':{'visibility':false,'value':null},
            //'nom_usuario':{'visibility':false,'value':null},
   ...
   methods: {
      ...
      asignar_recursos: function asignar_recursos(response) {
         ...
         this.lista_objs_model.map(function (lom) {
      ...
      eliminar_usuario_servicio: function eliminar_usuario_servicio(id) {
         ...
         this.$http.delete('/servicios_usuarios/' + id).then(function (response) {
            if (response.status == 200) {
               _this.eliminar_de_array_por_modelo_e_id(id, _this.servicio.servicios_usuarios, 'servicio_usuario');
      ...
      eliminar_bitacora_usuario: function eliminar_bitacora_usuario(id) {
         ...
         __WEBPACK_IMPORTED_MODULE_0_sweetalert2___default()({
            ...
            if (result.value) {
               ...
               _this2.$http.delete('/usuarios_bitacora_servicios/' + id).then(function (response) {
                  if (response.status == 200) {
                     _this2.eliminar_de_array_por_modelo_e_id(id, _this2.servicio.usuarios_bitacora_servicios, 'usuario_bitacora_servicio');
                     //this.auto_alerta_corta("Eliminado!", "Registro eliminado correctamente", "success", 800);
                  } else {
                     _this2.checkear_estado_respuesta_http(response.status);
                     return false;
            ...
            if (res == true) {
               ...
               _this4.$http.post('/usuarios_bitacora_servicios', formData).then(function (response) {
                  ...
                  if (response.status == 200) {
                     ...
                     _this4.servicio_nueva_bitacora.asunto = null;
                     _this4.servicio_nueva_bitacora.det_bitacora = null;
                     _this4.servicio.usuarios_bitacora_servicios.push(response.body.usuario_bitacora_servicio);
                     /*
                     this.inicializar();
                     */
                  } else {
