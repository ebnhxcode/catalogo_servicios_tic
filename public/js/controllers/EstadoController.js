...
var EstadoController = new Vue({
   ...
   watch: {
      ...
      estados: function estados(_estados) {
         var self = this;
         this.excel_json_datos = [];
         return _estados.map(function (estado, index) {
   ...
   methods: {
      ...
      inicializar: function inicializar() {
         ...
         this.$http.get('/estados').then(function (response) {
            // success callback
            _this.estados = response.body.estados || null;
            _this.datos_excel = response.body.estados || null;
      ...
      editar: function editar(id_estado) {
      ...
      guardar_editado: function guardar_editado() {
         ...
         this.$http.put('/' + this.nombre_ruta + '/' + this.estado.id_estado, this.estado).then(function (response) {
            // success callback

            if (response.status == 200) {
               if (!_this2.es_null(response.body.estado)) {
      ...
      eliminar: function eliminar(id_estado) {
         var _swal,
             _this3 = this;
         ...
         __WEBPACK_IMPORTED_MODULE_0_sweetalert2___default()((_swal = {
            ...
            if (result.value) {
               ...
               _this3.$http.delete('/' + _this3.nombre_ruta + '/' + id_estado).then(function (response) {
                  if (response.status == 200) {
      ...
      ordenar_lista: function ordenar_lista(columna) {
         this.estados = _.orderBy(this.estados, columna, this.orden_lista);
      }
